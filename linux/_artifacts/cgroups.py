...
def _cgroup_path(cgroup) -> str:
    ...
    try:
        if (
            not cgroup
            or not artifact_core._object_readable(cgroup)
            or not cgroup.has_member("kn")
            or not cgroup.kn
            or not artifact_core._object_readable(cgroup.kn)
        ):
            return ""
        node = cgroup.kn.dereference()
        root_node = cgroup.root.cgrp.kn
        if not root_node or not artifact_core._object_readable(root_node):
            raise ValueError("unreadable hierarchy root kernfs node")
        root_address = artifact_core._object_address(root_node)
    ...
    for _ in range(256):
        try:
            address = artifact_core._object_address(node)
            if address in seen:
...
def _cgroup_hierarchy(
    cgroup,
    default_root_address=None,
    *,
    errors=(
        "unreadable cgroup hierarchy root",
    ...
    if not cgroup or not artifact_core._object_readable(cgroup):
        raise ValueError("unreadable cgroup")
    root = cgroup.root
    if not root or not artifact_core._object_readable(root):
        raise ValueError(errors[0])
    root_address = artifact_core._object_address(root)
    ...
    try:
        ...
        for _ in range(4096):
            ...
            if artifact_core._object_address(
                link.cset
            ) != artifact_core._object_address(css_set):
                raise ValueError("cgrp_links entry points to another css_set")
            if artifact_core._object_address(link.cgrp_link.prev) != previous:
...
def _cgroup_memberships(task, issues_out=None) -> tuple[CgroupMembership, ...]:
    ...
    def report_issues():
        ...
        try:
            ...
            pid = "?"
        vollog.warning(
            "Incomplete cgroup metadata for PID %s: %s", pid, "; ".join(sorted(issues))
        )

    try:
        css_set = task.cgroups
        if not css_set or not artifact_core._object_readable(css_set):
    ...
    def add_group(cgroup):
        ...
        address = artifact_core._object_address(cgroup)
    ...
    for index in range(min(subsystem_count, 256)):
        try:
            ...
            if not css_pointer:
                continue
            if not artifact_core._object_readable(css_pointer):
            ...
            if entry["address"] != artifact_core._object_address(cgroup):
                conflicting_roots.add(root_address)
                issues.add("cgroup-id-conflict")
            ...
            if (
                css.has_member("ss")
                and css.ss
                and artifact_core._object_readable(css.ss)
            ):
                subsystem = css.ss.dereference()
                field = (
                    "legacy_name"
                    if subsystem.has_member("legacy_name") and subsystem.legacy_name
                    else "name"
                )
                if subsystem.has_member(field) and subsystem.member(field):
                    controller = artifact_core._read_kernel_cstring(
                        subsystem.member(field), max_bytes=64
                    )
...
def read_cgroup_chain(start, limit=128, parent_field=None):
    ...
    while current is not None:
        ...
        name = (
            utility.pointer_to_string(node.name, 256, errors="strict")
            if int(node.name)
            else ""
        )
        ...
        if selected_parent is None:
            selected_parent = (
                "parent"
                if node.has_member("parent")
                else "__parent"
                if node.has_member("__parent")
                else None
            )
        if selected_parent is None or not node.has_member(selected_parent):
            raise ValueError(
                "kernfs_node.parent|__parent: unsupported kernfs parent layout"
            )
...
class CgroupV2Resolver:
    ...
    def __init__(self, kernel_module, identify):
        ...
        templates = {
            name: artifact_core.require_fields(kernel_module, name, fields, feature)
        ...
        if self.parent_field is None:
            raise artifact_core.UnsupportedLayoutError(
                feature,
                "kernfs_node.parent|__parent",
                "no supported kernfs parent member",
            )
        self.compatibility = {
            "feature": feature,
            "status": "ok",
...
class CgroupLinksResolver:
    ...
    def __init__(self, module, identify):
        ...
        fields = {
            ...
            "cgroup": ("kn", "self", "root"),
            "cgroup_root": ("hierarchy_id",),
            "cgroup_subsys_state": ("parent", "cgroup"),
            "kernfs_node": ("name",),
            "list_head": ("next", "prev"),
        }
        types = {
            name: artifact_core.require_fields(module, name, names, feature)
        ...
        if self.parent_field is None:
            raise artifact_core.UnsupportedLayoutError(
                feature, "kernfs_node.parent|__parent", "unsupported parent layout"
            )
        ...
        if address not in self.cache:
            ...
            while current != head_address:
                if not current or current in seen or len(seen) >= 256:
                    raise ValueError(
                        "Broken, cyclic or oversized css_set.cgrp_links list"
                    )
                seen.add(current)
                link = self.module.object(
                    "cgrp_cset_link", offset=current - self.link_offset, absolute=True
                )
                if (
                    int(link.cgrp_link.vol.offset) != current
                    or int(link.cgrp_link.prev) != previous
                ):
                ...
                evidence.append(
                    {
                        ...
                        "hierarchy": "v2" if hierarchy == 0 else "v1",
...
class CgroupPathPolicy(enum.Enum):
    INVENTORY = "inventory"
    NETWORK = "network"
    ...
    while node or not inventory:
        ...
        if inventory:
            if address in seen or len(seen) >= limit:
                raise artifact_core.Incomplete("cgroup parent cycle/budget")
        else:
            if address in seen:
        ...
        seen.add(address)
        if modern and not inventory or node.has_member("name"):
            parts.append(string(node.name))
        elif node.has_member("name_copy") and (inventory or node.name_copy):
            parts.append(
                string(node.name_copy)
                if inventory
                else utility.array_to_string(node.name_copy.name)
            )
        else:
            raise (artifact_core.Unsupported if inventory else NotImplementedError)(
                "cgroup name layout unavailable"
            )
        if inventory:
            parent = (
                node.member("__parent")
                if node.has_member("__parent")
                else node.parent
                if node.has_member("parent")
                else node.self.parent.cgroup
                if not modern and node.self.parent
                else None
            )
            trace.append(
                {
...
def _file_cgroup_hierarchy(cgroup, default_root=None):
    return _cgroup_hierarchy(
        ...
        default_root,
        errors=(
            "unreadable cgroup root",
            "negative hierarchy ID",
            "default hierarchy address/ID mismatch",
        ),
    )
...
def read_membership_path(cgroup):
    ...
    parts, seen = [], set()
    for _ in range(256):
        if not node or not artifact_core._object_readable(node):
        ...
        if address == root_address:
            if artifact_core._object_address(parent):
                raise ValueError("hierarchy root has a parent")
            return "/" + "/".join(reversed(parts))
        if not parent or not artifact_core._object_readable(parent):
            raise ValueError("kernfs ancestry ended before hierarchy root")
        name = artifact_core.read_file_cstring(node.name, max_bytes=256)
        if (
            not name
            or name in {".", ".."}
            or "/" in name
...
def _file_cgroup_links(css_set, issues):
    ...
    try:
        ...
        context = head._context
        type_name = head.vol.type_name.split("!", 1)[0] + "!cgrp_cset_link"
        field_offset = context.symbol_space.get_type(type_name).relative_child_offset(
            "cgrp_link"
        )
        head_address = artifact_core._object_address(head)
        previous, pointer, seen = head_address, head.next, {head_address}
        for index in range(4097):
            address = artifact_core._object_address(pointer)
            ...
            if not pointer or not artifact_core._object_readable(pointer):
            ...
            if not artifact_core._object_readable(link):
...
def read_link_memberships(task, issues_out=None, *, logger=None):
    ...
    def finish(memberships):
        if issues:
            ...
            (logger or logging.getLogger(__name__)).debug(
                "Partial task cgroups: %s", "; ".join(sorted(issues))
            )
        return tuple(memberships)
    ...
    for cgroup in groups:
        try:
            ...
            if root_address in entries and entries[root_address]["address"] != address:
                ...
                continue
            if (
                hierarchy_id in hierarchy_roots
                and hierarchy_roots[hierarchy_id] != root_address
            ):
    ...
    for index in range(min(subsystem_count, 256)):
        try:
            ...
            if not css:
                continue
            if not artifact_core._object_readable(css):
                raise ValueError("unreadable controller state")
            version, root_address, _, _ = _file_cgroup_hierarchy(
                css.cgroup, default_root
            )
            if version == "v2":
                continue
            if root_address not in entries:
                ...
                continue
            entry = entries[root_address]
            if entry["address"] != artifact_core._object_address(css.cgroup):
                conflicting_roots.add(root_address)
                issues.add("cgroup-id-conflict")
                continue
            controller = f"subsys-{index}"
            if css.has_member("ss") and css.ss:
                subsystem = css.ss
                if not artifact_core._object_readable(subsystem):
                ...
                controller = artifact_core.read_file_cstring(
                    subsystem.member(field), max_bytes=64
                )
            if (
                not controller
    ...
    for root_address, entry in sorted(entries.items()):
        ...
        if entry["version"] == "v1":
            try:
                root = entry["root"]
                name = (
                    utility.array_to_string(root.name)
                    if root.has_member("name")
                    else ""
                )
                if name:
