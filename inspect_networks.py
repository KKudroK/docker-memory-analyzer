...
def identity_collect(c, records):
    ...
    for ns in c.nets.values():
        candidates = set()
        for record in records:
            if record.get("namespace") == ns["address"]:
                candidates.update(record["container_candidates"])
...
def sockets_expand_unix_peers(c, unique, selected_pids):
    ...
    seeds = [
        ...
        if row["family"] == 1 and any(h["pid"] in selected_pids for h in row["holders"])
    ]
    for row in seeds:
        peer = row.get("peer")
        ...
        def read_peer(*, peer=peer, row=row):
        ...
        def task_fds(*, task=task):
            ...
            for fd in range(count):
                ...
                def entry(*, fd=fd):
                    ...
                    if not inode:
                        raise ValueError(f"socket file {address:#x} has NULL d_inode")
...
def context_build(report):
    ...
    for sid, sock in sockets.items():
        ...
        for holder in holders:
            holder_ids.update(holder["container_ids"])
        cids = sorted(holder_ids)
        ownership[sid] = {"container_ids": cids, "holders": holders}
        for cid in cids:
            contexts[cid]["socket_ids"].append(sid)
            if any(
                h["container_ids"] == [cid] and h["task_namespace"] != sock["namespace"]
                for h in holders
            ):
...
class Collector(artifact_core.CollectionSession):
    ...
    def walk(self, head, typename, member):
        return task_readers.walk_list(
            self, head, typename, member, check_backlinks=False
        )
    ...
    def cgroups(self, task):
        ...
        for group in groups:
            path = self.read(
                "cgroup.path", group, lambda group=group: self.cgroup_path(group)
            )
    ...
    def discover_tasks(self):
        ...
        try:
            ...
            hidden_leaders = psscan.PsScan.scan_tasks(
                self.context, self.kernel.name, self.kernel.layer_name
            )
            self.read(
                "tasks.psscan",
                "PsScan.scan_tasks",
    ...
    def interface(self, net, dev):
        ...
        for field in ("mtu", "flags", "promiscuity", "operstate", "type"):
            row[field] = self.read(
                "interface." + field, dev, lambda field=field: int(dev.member(field))
            )
        ...
        if dev.has_member("rtnl_link_ops") and dev.rtnl_link_ops:
            row["kind"] = self.read(
                "interface.kind",
                dev,
                lambda: network_readers.link_kind(dev, self.cstring),
            )
        ...
        def ipv4():
            ...
            for _ in range(self.limit):
                ...
                ifa = ptr.dereference()
                value = self.read(
                    "ipv4.entry", ifa, lambda ifa=ifa: self.address(ifa, 4)
                )
                if value:
        ...
        def ipv6():
            ...
            for ifa in self.walk(ipdev.addr_list, "inet6_ifaddr", "if_list"):
                value = self.read(
                    "ipv6.entry", ifa, lambda ifa=ifa: self.address(ifa, 6)
                )
                if value:
                    row["addresses"].append(value)
...
class InspectNetworks(interfaces.plugins.PluginInterface):
    ...
    def run(self):
        ...
        return _vertical.vertical_grid(self._columns(), self._generator(report))
