...
class ContainerTasks(interfaces.plugins.PluginInterface):
    ...
    def run(self):
        ...
        if prefix and not re.fullmatch(r"[0-9a-fA-F]{6,64}", prefix):
            raise exceptions.VolatilityException("Container prefix must be 6-64 hexadecimal characters")
        ...
        if prefix and len({cid for cid in observed_ids if cid.startswith(prefix)}) > 1:
            raise exceptions.VolatilityException("Container prefix is ambiguous; supply more characters")
