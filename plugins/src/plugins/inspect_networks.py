# Generated from the maintained InspectNetworks implementation.
# Public export: English docstrings and concise implementation comments.
...
def identity_cgroup_candidates(paths):
    ...
    for path in paths:
        ...
        for index, part in enumerate(parts):
            scope = re.fullmatch(
                "(?:docker|cri-containerd|crio|libpod)-([0-9a-f]{64})\\.scope", part
            )
            if scope:
                result.add(scope.group(1))
            elif (
                index
                and parts[index - 1] == "docker"
                and re.fullmatch("[0-9a-f]{64}", part)
            ):
                result.add(part)
...
def identity_member_id(task):
    # An ancestor shim or a shared namespace alone must never establish membership.
    ...
    if (
        len(ids) != 1
        or task.get("identity_conflict")
        or task.get("cgroup_complete") is False
        or task.get("pid_conflict")
    ):
        return None
...
def identity_collect(c, records):
    # Runtime arguments supplement cgroups; conflicting evidence remains unresolved.
    ...
    for task in c.tasks:
        ...
        record["parent"] = c.read(
            "task.parent", task, identity_parent_address, args=(task,)
        )
        comm = record["comm"]
            ...
            if address == node["address"]:
                if node.get("pid") != 0:
                    c.issue(
                        "identity.ancestry",
                        node["address"],
                        ValueError("non-root task is its own parent"),
                    )
                break
        ...
        ns["container_candidates"] = sorted(
            {
                cid
                for r in records
                if r.get("namespace") == ns["address"]
                for cid in r["container_candidates"]
            }
        )
...
def sockets_read_sock(c, sk, source):
    ...
    c.read(
        "socket.namespace",
        sk,
        lambda: c.namespace(c.obj("net", ns), "sock.__sk_common.skc_net"),
    )
    row = {
        "socket": hex(sk.vol.offset),
        "namespace": hex(int(ns)),
        "family": int(common.skc_family),
        "source": source,
        "receive_queue_packets": int(sk.sk_receive_queue.qlen),
        "state": int(common.skc_state),
        "socket_type": int(sk.sk_type),
        "protocol_number": int(sk.sk_protocol),
    }
    if row["family"] in (2, 10):
        inet = sk.cast("inet_sock")
        row.update(
            protocol=str(inet.get_protocol()),
            source_port=int(inet.get_src_port()),
            destination_port=int(inet.get_dst_port()),
        )
        if row["family"] == 10:
            row["source_ip"] = socket.inet_ntop(
                socket.AF_INET6, c.layer.read(common.skc_v6_rcv_saddr.vol.offset, 16)
            )
            row["destination_ip"] = socket.inet_ntop(
                socket.AF_INET6, c.layer.read(common.skc_v6_daddr.vol.offset, 16)
            )
        else:
        ...
        if unix.addr:
            ...
            path_offset = c.kernel.get_type("sockaddr_un").relative_child_offset(
                "sun_path"
            )
            if not path_offset <= length <= c.kernel.get_type("sockaddr_un").size:
                raise ValueError("invalid sockaddr_un length")
            raw = c.layer.read(
                unix.addr.name.vol.offset + path_offset, length - path_offset
            )
            row["path_bytes_hex"] = raw.hex()
            row["abstract"] = raw.startswith(b"\x00")
            row["path"] = (
                "@" + raw[1:].decode("utf-8", errors="backslashreplace")
                if row["abstract"]
                else raw.rstrip(b"\x00").decode("utf-8", errors="backslashreplace")
            )
...
def sockets_merge_holders(rows):
    # Keep every holder when several descriptors resolve to the same socket.
    unique = {}
    for row in rows:
        holder = {key: row[key] for key in ("pid", "fd", "file")}
        if row["socket"] not in unique:
            unique[row["socket"]] = dict(
                {
                    key: value
                    for key, value in row.items()
                    if key not in ("pid", "fd", "file")
                },
                holders=[],
            )
...
def sockets_expand_unix_peers(c, unique, selected_pids):
    ...
    seeds = [
        ...
        if row["family"] == 1
        and any((h["pid"] in selected_pids for h in row["holders"]))
    ]
    for row in seeds:
        peer = row.get("peer")
        ...
        def read_peer():
            ...
            result.update(
                holders=[], confidence="reciprocal_kernel_peer", discovery="unix_peer"
            )
...
def sockets_collect(c, task_records=None):
    """Follows task file descriptors to sockets and retains each observed holder."""
    # An FD proves a local holder, not ownership of the remote network endpoint.
        ...
        def task_fds():
            ...
            for fd in range(count):
                ...
                def entry():
                    ...
                    alloc = linux.LinuxUtilities.container_of(
                        int(inode), "socket_alloc", "vfs_inode", c.kernel
                    )
                    if alloc is None:
                        if inode:
                            offset = c.kernel.get_type(
                                "socket_alloc"
                            ).relative_child_offset("vfs_inode")
                    ...
                    result = sockets_read_sock(
                        c, sk, "task.files -> fd -> file -> socket_alloc.socket.sk"
                    )
                    result.update(
                        confidence="fd_reachable_socket_object", discovery="fd"
                    )
...
def conntrack_symbols(c, names):
    found = {
        n: c.kernel.object_from_symbol(n).vol.offset
        for n in names
        if c.kernel.has_symbol(n)
    }
    ...
    def modules():
        for mod in c.walk(c.kernel.object_from_symbol("modules"), "module", "list"):
            ...
            for sym in c.array(int(ks.symtab), sym_type, int(ks.num_symtab)):
                ...
                name = (
                    c.layer.read(int(ks.strtab) + offset, 128)
                    .split(b"\x00", 1)[0]
...
def conntrack_tuple_data(c, value):
    ...
    row = {
        ...
        "destination_ip": socket.inet_ntop(
            af, c.layer.read(value.dst.u3.vol.offset, size)
        ),
    }
    if protocol in (6, 17, 33, 132, 136):
        row["source_port"] = int.from_bytes(
            c.layer.read(value.src.u.vol.offset, 2), "big"
        )
        row["destination_port"] = int.from_bytes(
            c.layer.read(value.dst.u.vol.offset, 2), "big"
        )
    return row


def conntrack_collect(c, unsupported):
    """Reads connection-tracking entries from supported symbol-backed hash tables."""
    # NAT flags describe tracked kernel state, not proof of completed communication.
    rows = []
    ...
    if missing:
        unsupported.append(
            {
                "feature": "conntrack",
                "reason": "ISF types absent: " + ", ".join(missing),
            }
        )
        return rows
    ...
    if any((n not in found for n in names)):
        unsupported.append(
            {
                "feature": "conntrack",
                "reason": "hash symbols absent in kernel/module kallsyms",
            }
        )
        return rows
    ...
    def scan():
        ...
        entry_offset = c.kernel.get_type(
            "nf_conntrack_tuple_hash"
        ).relative_child_offset("hnnode")
        ...
        for head in heads:
            ...
            def bucket():
                ...
                while link and (not link & 1):
                    ...
                    ct = c.obj(
                        "nf_conn",
                        entry.vol.offset - tuples_offset - direction * tuple_size,
                    )
                    ...
                    def record():
                        ...
                        if (int(original.src.l3num), int(original.dst.protonum)) != (
                            int(reply.src.l3num),
                            int(reply.dst.protonum),
                        ):
                            raise ValueError("conntrack tuple family/protocol mismatch")
                        rows.append(
                            {
                                "address": hex(ct.vol.offset),
                                "namespace": hex(namespace),
                                "original": conntrack_tuple_data(
                                    c, ct.tuplehash[0].tuple
                                ),
                                "reply": conntrack_tuple_data(c, ct.tuplehash[1].tuple),
                                "status": status,
                                "snat": bool(status & 1 << 4),
                                "dnat": bool(status & 1 << 5),
                                "source": "nf_conntrack_hash -> tuplehash -> nf_conn",
                                "confidence": "hashed_flow_object",
...
def context_build(report):
    ...
    for sid, sock in sockets.items():
        ...
        for h in sock["holders"]:
            ...
            holders.append(
                dict(
                    h,
                    tgid=t.get("tgid", h["pid"]),
                    container_ids=context_task_ids(t),
                    task_namespace=t.get("namespace"),
                    identity_conflict=t.get("identity_conflict", False),
                )
            )
        ...
        for cid in cids:
            ...
            if any(
                (
                    h["container_ids"] == [cid]
                    and h["task_namespace"] != sock["namespace"]
                    for h in holders
                )
            ):
        ...
        relations.append(
            {
                ...
                "left_socket": left,
                "right_socket": right,
                "left_containers": a["container_ids"],
                "right_containers": b["container_ids"],
                "left_holders": a["holders"],
                "right_holders": b["holders"],
                "evidence": evidence,
    ...
    for sid, sock in sockets.items():
        ...
        if (
            ...
            and peer in sockets
            and (sid < peer)
            and (sockets[peer]["family"] == 1)
            and (sockets[peer].get("peer") == sid)
        ):
            relation(
                "unix_peer",
                sid,
    ...
    for address, ns in nets.items():
        if address in (None, "", "0x0", 0):
            continue
        members = sorted(
            (cid for cid, item in contexts.items() if address in item["namespaces"])
        )
        if len(members) > 1:
            structural.append(
                {
    ...
    for sid, owner in ownership.items():
        if len(owner["container_ids"]) > 1:
            structural.append(
                {
                    "kind": "shared_socket",
                    "members": owner["container_ids"],
                    "evidence": [sid],
                    "confidence": "same_fd_reachable_socket",
                }
            )
    for item in contexts.values():
    ...
    return {
        "containers": [contexts[k] for k in sorted(contexts)],
...
class Collector:
    """Collects linked kernel objects and records partial-read failures with their evidence scope."""
    ...
    def issue(self, stage, obj, exc):
        """Records a failed read with its stage and object address for later diagnostics."""
        self.errors.append(
            {
                "stage": stage,
    ...
    def read(self, stage, obj, fn, default=None, *, args=()):
        """Runs a scoped read and records a failure before returning the requested fallback."""
        try:
            return fn(*args)
    ...
    def array(self, address, typename, count):
        ...
        return self.kernel.object(
            ...
            offset=int(address),
            absolute=True,
            subtype=self.kernel.get_type(typename),
            count=count,
        )
    ...
    def namespace(self, net, source):
        ...
        if address not in self.nets:
            ...
            self.nets[address]["inode"] = self.read(
                ...
                net,
                lambda: (
                    int(net.ns.inum) if net.has_member("ns") else int(net.proc_inum)
                ),
            )
    ...
    def cgroup_path(self, group):
        ...
        while True:
            ...
            parent = (
                node.parent if node.has_member("parent") else node.member("__parent")
            )
            if not parent:
    ...
    def discover_tasks(self):
        ...
        self.tasks = []
        seen = set()
        leaders = pslist.PsList.list_tasks(
    ...
    def collect_tasks(self):
        ...
        for task in self.tasks:
            ...
            proxy = self.read(
                "task.nsproxy",
                task,
                lambda: task.nsproxy.dereference() if task.nsproxy else None,
            )
            if proxy is not None:
                net = self.read(
                    "task.net",
                    task,
                    lambda: proxy.net_ns.dereference() if proxy.net_ns else None,
                )
                if net is not None:
            ...
            for field in ("start_time", "start_boottime"):
                if task.has_member(field):
                    record[field] = self.read(
                        "task." + field, task, lambda f=field: int(task.member(f))
                    )
        ...
        for pid, duplicates in by_pid.items():
            if len(duplicates) > 1:
                ...
                self.issue(
                    ...
                    str(pid),
                    ValueError(
                        "distinct task objects share one PID; membership excluded"
                    ),
                )
        return records
    ...
    def interface(self, net, dev):
        row = {
            "address": hex(dev.vol.offset),
            "namespace": net["address"],
            "namespace_inode": net["inode"],
            "container_candidates": net["container_candidates"],
            "pids": net["pids"],
            "name": utility.array_to_string(dev.name),
            "ifindex": int(dev.ifindex),
            "addresses": [],
            "source": "kernel.net.dev_base_head",
            "confidence": "direct_object_membership",
        }
        row["device_namespace"] = self.read(
            "interface.nd_net", dev, lambda: hex(self.net_pointer(dev.nd_net))
        )
        if (
            row["device_namespace"] is not None
            and row["device_namespace"] != net["address"]
        ):
            raise ValueError("dev_base_head and nd_net disagree")
        for field in ("mtu", "flags", "promiscuity", "operstate", "type"):
            row[field] = self.read(
                "interface." + field, dev, lambda: int(dev.member(field))
            )
        ...
        def mac():
            ...
            return ":".join(
                (f"{b:02x}" for b in self.layer.read(int(dev.dev_addr), size))
            )

        row["mac"] = self.read("interface.mac", dev, mac)
        if dev.has_member("rtnl_link_ops") and dev.rtnl_link_ops:
            row["kind"] = self.read(
                "interface.kind", dev, lambda: self.cstring(dev.rtnl_link_ops.kind)
            )
    ...
    def collect(self, features=None):
        ...
        if self.kernel.has_symbol("init_net"):
            initial = self.read(
                "net.init",
                "init_net",
                self.kernel.object_from_symbol,
                args=("init_net",),
            )
        ...
        for ns in self.nets.values():
            ...
            if (
                features & {"interfaces", "conntrack"}
                and ns["address"] in members
                and (ns["is_initial"] or not ns["initial_identity_known"])
            ):
                unsupported.append(
                    {
                        "feature": "container_namespace_context",
        ...
        self.context_namespaces = {
            ns["address"]
            for ns in self.nets.values()
            if ns["address"] in members
            and (not ns["is_initial"])
            and ns["initial_identity_known"]
        }
        interfaces = []
            ...
            for net in list(self.nets.values()):
                ...
                def devices():
                    for dev in self.walk(
                        net["object"].dev_base_head, "net_device", "dev_list"
                    ):
                        value = self.read(
                            "interface", dev, self.interface, args=(net, dev)
                        )
                        if value:
        ...
        socket_rows = (
            self.read(
                "socket.collect", "kernel", sockets_collect, [], args=(self, tasks)
            )
        ...
        flow_rows = (
            self.read(
                ...
                conntrack_collect,
                [],
                args=(self, unsupported),
            )
            if "conntrack" in features
            else []
        )
        for ns in self.nets.values():
            ns["is_initial"] = ns["address"] == self.init_net_address
        ...
        report = {
            ...
            "sockets": socket_rows,
            "metadata": {
                "kernel_symbol_table": self.kernel.symbol_table_name,
            ...
            "summary": {
                ...
                "addresses": sum((len(i["addresses"]) for i in interfaces)),
...
def views_scope(report, prefixes=None):
    # Host-network members keep FD evidence; host-wide tables are not container-owned.
    ...
    if prefixes:
        ...
        for prefix in prefixes:
            ...
            if len(matches) != 1:
                raise ValueError(
                    f"Container prefix {prefix!r} matched {len(matches)} IDs"
                )
...
def views_columns(view):
    ...
    schemas = {
        ...
        "conntrack": common
        + [("Proto", str), ("Source", str), ("Destination", str), ("NAT", str)],
        "diagnostics": [
            ("Kind", str),
            ("Stage", str),
            ("Object", str),
...
def views_rows(report, view, prefixes=None):
    """Renders collected namespace evidence without assigning endpoint ownership."""
    ...
    if view == "diagnostics":
        ...
        for item in report.get("unsupported", []):
            yield (
                ...
                (
                    "unsupported",
                    views_text(item["feature"]),
                    str(item.get("namespace", item.get("interface", ""))),
    ...
    if view == "containers":
        for cid in sorted(selected):
            ...
            sockets = {
                ...
                if any((h["pid"] in pids for h in s["holders"]))
            }
    ...
    for item in report.get(view, []):
        ...
        if ns.get("is_initial") or not ns.get("initial_identity_known", True):
            continue
        common = (
            ", ".join((labels[cid] for cid in sorted(cids))),
            views_value(ns.get("inode")),
        )
        if view == "interfaces":
            for ip in item.get("addresses") or [{}]:
                cidr = (
                    ip["ip"] + "/" + str(ip["prefix"])
                    if ip.get("ip") is not None and ip.get("prefix") is not None
                    else ip.get("ip")
                )
                yield (
                    0,
                    common
                    + (
                        views_text(item["name"]),
                        views_value(cidr),
                        views_value(item.get("mac")),
                    ),
                )
        elif view == "conntrack":
            orig = item["original"]
            proto = {1: "ICMP", 6: "TCP", 17: "UDP", 58: "ICMPv6", 132: "SCTP"}.get(
                orig["protocol"], f"IPPROTO_{orig['protocol']}"
            )
            ...
            nat = "+".join(
                (
                    name
                    for name, key in [("SNAT", "snat"), ("DNAT", "dnat")]
                    if item[key]
                )
            )
            yield (
                ...
                + (
                    proto,
                    views_endpoint(orig, "source"),
                    views_endpoint(orig, "destination"),
                    nat or "None",
                ),
            )
...
class InspectNetworks(interfaces.plugins.PluginInterface):
    """Lists container-owned sockets or direct shared-object and UNIX peer relations."""
    ...
    def get_requirements(cls):
        """Returns the kernel dependencies and supported collection options."""
        return [
            requirements.ModuleRequirement(
                name="kernel",
                description="Linux kernel (validated x86-64 scope)",
                architectures=["Intel64"],
            ),
            requirements.VersionRequirement(
                name="net_symbols", component=network.NetSymbols, version=(1, 0, 0)
            ),
            requirements.VersionRequirement(
            ...
            requirements.ListRequirement(
                name="container",
                description="Filter by unambiguous observed ID reference(s) or prefix(es); not Docker inventory",
                element_type=str,
                optional=True,
            ),
            requirements.ChoiceRequirement(
                name="view",
                description="Container evidence view; diagnostics checks all retained collectors",
                choices=[
                    "sockets",
                    "relations",
                    "containers",
            ...
            requirements.BooleanRequirement(
                name="dump-evidence",
                description="Save evidence for this view; does not enable extra collectors",
                default=False,
                optional=True,
            ),
        ]
    ...
    def run(self):
        """Collects the requested evidence and returns a TreeGrid for the selected renderer."""
        # TreeGrid views consume collected evidence; exports are explicitly requested.
        ...
        report = Collector(self.context, self.config["kernel"]).collect(
            features=features_by_view[view]
        )
        known = {
            cid for task in report["tasks"] if (cid := plugin_container_member_id(task))
        }
        ...
        if view == "relations":
            return renderers.TreeGrid(
                plugin_relation_columns(),
                plugin_relation_rows(report, self.config.get("container")),
            )
        if view != "sockets":
            return renderers.TreeGrid(
                views_columns(view),
                views_rows(report, view, self.config.get("container")),
            )
        return renderers.TreeGrid(self._columns(), self._generator(report))

    def _log_diagnostics(self, report):
        """Logs parse failures and identity conflicts separately from unsupported layouts."""
        for error in report.get("errors", []):
            vollog.warning(
                "Parse error stage=%s object=%s type=%s detail=%s affected_holders=%s",
                error["stage"],
                error["address"],
                error["error"],
                error["detail"],
        ...
        for item in report.get("unsupported", []):
            ...
            log = (
                vollog.warning
                if item["feature"] in requested
                or item["feature"] == "container_namespace_context"
                or "all_supported" in requested
                else vollog.debug
            )
            log(
                "Unsupported feature=%s object=%s reason=%s",
                item["feature"],
                item.get("interface", item.get("namespace", "")),
                item["reason"],
            )
        for address in report["container_context"].get("unresolved_tasks", []):
            vollog.warning(
                "Conflicting or multiple ID references for task %s; ID not assigned",
                address,
            )

    @staticmethod
    def _columns():
        """Returns the fixed column schema for the container socket summary."""
        return [
            ("Container ID", str),
            ("NetNS", int),
            ("Proto", str),
    ...
    def _generator(self, report):
        """Yields container-owned socket rows, collapsing identical per-process FD observations."""
        ...
        for sock in report["sockets"]:
            for holder in sock["holders"]:
                ...
                yield (
                    ...
                    (
                        labels[cid],
                        plugin_namespace_inode(nets, sock["namespace"]),
                        proto,
                        task["tgid"],
                        task["comm"],
                        holder["fd"],
                        plugin_socket_endpoint(sock, "source"),
                        plugin_socket_endpoint(sock, "destination"),
                        state,
                    ),
                )
...
def plugin_container_member_id(task):
    """Returns a unique ID from the task cgroup, excluding unresolved or conflicting membership."""
    return identity_member_id(task)


def plugin_select_containers(known, prefixes):
    """Resolves ID prefixes and produces collision-free display labels."""
    selected = set(known)
    if prefixes:
        selected = set()
        for prefix in prefixes:
            ...
            if len(matches) != 1:
                raise ValueError(
                    f"Container prefix {prefix!r} matched {len(matches)} IDs; use a longer/existing ID"
                )
            selected.update(matches)
...
def plugin_relation_columns():
    """Returns the fixed column schema for direct kernel-object relations."""
    return [
        ("Relation", str),
        ("Container ID", str),
        ("Object", format_hints.Hex),
        ("Peer Container", str),
        ("Peer Object", format_hints.Hex),
        ("Evidence", str),
    ]


def plugin_relation_rows(report, prefixes=None):
    """Yields shared-object memberships and reciprocal UNIX peers without inferring traffic."""
    # Shared objects and UNIX peers are relations, not reconstructed traffic flows.
    tasks = []
    for task in report["tasks"]:
        cid = plugin_container_member_id(task)
        if cid:
            tasks.append(
                dict(task, container_candidates=[cid], identity_conflict=False)
            )
    pids = {task["pid"] for task in tasks}
    sockets = [
        dict(sock, holders=[h for h in sock["holders"] if h["pid"] in pids])
        for sock in report["sockets"]
    ]
            ...
            if cid in selected:
                yield (
                    ...
                    (
                        item["kind"],
                        labels[cid],
                        plugin_address_value(item["evidence"][0]),
        ...
        for left in item["left_containers"] or [None]:
            for right in item["right_containers"] or [None]:
                ...
                yield (
                    ...
                    (
                        item["kind"],
                        plugin_value_or_absent(labels.get(left)),
                        plugin_address_value(item["left_socket"]),
                        plugin_value_or_absent(labels.get(right)),
                        plugin_address_value(item["right_socket"]),
                        item["confidence"],
                    ),
                )
...
def plugin_socket_endpoint(sock, side):
    """Formats a socket endpoint without modifying the collected address or port fields."""
    if sock["family"] == 1:
        if side == "destination":
            return renderers.NotApplicableValue()
        path = sock.get("path")
        return plugin_value_or_absent(
            path, lambda value: value.encode("unicode_escape").decode("ascii")
        )
...
def plugin_socket_labels(sock):
    """Resolves protocol and state labels without applying TCP semantics to other protocols."""
    ...
    if family in (2, 10):
        ...
        return (
            label,
            (
                states.get(state, str(state))
                if protocol == 6 and state is not None
                else plugin_value_or_absent(state, str)
            ),
        )
    return (
        f"AF_{family}/PROTO_{protocol}",
        plugin_value_or_absent(sock.get("state"), str),
    )
