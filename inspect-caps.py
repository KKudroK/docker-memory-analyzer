...
from volatility3.plugins.linux._artifacts.cgroups import (
    membership_resolver as make_membership_resolver,
    read_cgroup_chain,
    CgroupV2Resolver,
    CgroupLinksResolver,
)
...
from volatility3.plugins.linux._artifacts.credentials import (
    CAP_FIELDS,
...
from volatility3.plugins.linux._artifacts.namespaces import (
    namespace_inum as read_namespace_inum,
    _pidtype_pid,
    inspect_pid_layout,
    read_pid_chain,
)
...
def membership_resolver(module):
    return make_membership_resolver(module, identify_docker)


def namespace_inum(namespace):
    return read_namespace_inum(namespace, member_reader=member,
        missing_error=UnsupportedLayout('Neither namespace.ns.inum nor namespace.proc_inum exists'))
...
class ContainerCaps(interfaces.plugins.PluginInterface):
    ...
    def run(self):
        ...
        try:
            ...
            tasks = list_tasks(self.context, self.config['kernel'], include_threads=audit['include_threads'])
            for task in tasks:
        ...
        if self.config.get('view', 'raw') == 'analyst':
            ...
            return vertical_grid([(name, str) for name in SUMMARY_COLUMNS],
        ...
        def generate():
            for member in audit['unresolved_members'] if show_unresolved else selected:
                ...
                yield 0, values
        return vertical_grid(columns, generate())
